...
AI-assisted E2E testing with 3 specialized agents:
...
claude "Use the planner agent to create a test plan for [feature]"

# Generate tests from a plan
claude "Use the generator agent to create tests from specs/[feature].md"

# Auto-heal broken tests
claude "Use the healer agent to fix failing tests"
```

See `tests/e2e/seed.spec.ts` for the initialization context.

#### Nightly E2E Healing

The `nightly-heal.yml` GitHub Action runs daily at 3 AM UTC:
1. Executes all E2E tests
2. If failures, launches the Healer Agent to auto-fix selectors
3. Creates a PR with fixes for human review

Manual trigger: `gh workflow run nightly-heal.yml`

Local usage: `pnpm test:e2e:ci-heal`
